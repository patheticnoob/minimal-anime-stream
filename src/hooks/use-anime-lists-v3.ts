...
export function useAnimeListsV3(isActive: boolean = true) {
  ...
  useEffect(() => {
    ...
    // Load recent episodes for the recent episodes section
    logInfo('Fetching recent episodes section from Yuma (v3 flow)', 'Initial Load');
    retryWithBackoff(() => fetchYumaEndpoint('recent-episodes', 1))
      .then((data) => {
        if (!mounted) return;
        ...
        logInfo('Recent episodes section loaded from Yuma (v3)', 'Initial Load');
      })
      .catch((err) => {
        if (!mounted) return;
        const msg = err instanceof Error ? err.message : "Failed to load recent episodes";
