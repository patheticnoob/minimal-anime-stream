...
export function usePlayerLogic(isAuthenticated: boolean, dataFlow: string = "v1") {
  ...
  }, [fetchServers, fetchSources, audioPreference, dataFlow]);
  ...
  useEffect(() => {
    ...
    const fetchWithRetry = async () => {
      try {
        ...
        const extractedDataId = selected.dataId!.split('-').pop() || selected.dataId!;
        eps = await fetchEpisodes({ dataId: extractedDataId }) as Episode[];
  ...
  const buildProxiedUrl = (originalUrl: string) => {
    ...
    return { base, proxiedUrl: `${base}/proxy?url=${encodeURIComponent(originalUrl)}` };
  };
  ...
  const playEpisode = async (episode: Episode, retryCount = 0) => {
    ...
    try {
      ...
      if (sourcesData.sources && sourcesData.sources.length > 0) {
        ...
        const originalUrl = m3u8Source?.file || sourcesData.sources[0].file;
        const { base, proxiedUrl } = buildProxiedUrl(originalUrl);
