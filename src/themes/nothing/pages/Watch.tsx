...
export default function NothingWatch() {
  ...
  useEffect(() => {
    ...
    // Always use HiAnime API for episodes, extract dataId from animeId
    const extractedDataId = animeId.split('-').pop() || animeId;
    ...
    if (dataFlow === "v2") {
      ...
      // Use Convex hianime API (v1 flow) for all other flows including v5
      fetchPromise = fetchEpisodes({ dataId: extractedDataId }).then((eps) => eps as Episode[]);
    }
  ...
  }, [animeId, fetchEpisodes, navigate, dataFlow, dataFlowLoading]);
